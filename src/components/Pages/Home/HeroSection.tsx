...
const HeroCanvas = lazy(() => import('./HeroCanvasR3F'));
...
// ─── Types ────────────────────────────────────────────────────────────────────
interface UserProfile { full_name?: string; avatar_url?: string; email?: string; }
        ...
        >
          ...
          flex items-center justify-center text-[9px] font-bold text-sys-text-secondary">
            ...
            <p className="relative z-10 text-[12px] md:text-[13px] leading-relaxed text-sys-text-secondary">
                ...
                exceptional product design ensured our website's success. Highly Recommended!
            </p>
    ...
    return (
        ...
        >
            <div className="rounded-2xl border border-white/10 p-4 md:p-5 backdrop-blur-xl"
                ...
                {/* 5-star row */}
                <div className="flex gap-1 mb-2" role="img" aria-label="5 out of 5 stars">
                    {Array.from({ length: 5 }).map((_, i) => <Star key={i} delay={0.6 + i * 0.08} />)}
                </div>
                ...
                <div className="flex gap-4">
                    {[{ val: '40+', label: 'Projects' }, { val: '98%', label: 'Satisfaction' }].map(({ val, label }) => (
...
const ScrollIndicator = () => (
    ...
    >
        <p className="text-[10px] uppercase tracking-[0.25em] text-sys-text-secondary opacity-60">Scroll</p>
...
export default function HeroSection() {
    ...
    // ── Derived ──
    const firstName = profile?.full_name?.split(' ')[0] ?? 'Mekeshkumar';
    ...
    return (
        <section
            ref={sectionRef}
            aria-label="Hero — Mekeshkumar profile"
        ...
        >
            ...
            <header className="relative z-20 flex flex-col items-center w-full px-4 text-center">
                ...
                <motion.div initial="hidden" animate={show} variants={up(0.25)}
                    ...
                    {['EEE Student', 'UI/UX Designer', 'Full-Stack Dev'].map((tag, i) => (
                        <motion.span key={tag}
                ...
                <motion.p initial="hidden" animate={show} variants={up(0.3)}
                    ...
                    A pre-final year Electrical &amp; Electronics Engineering student at{' '}
                    <span className="text-sys-text-primary font-bold">Kongu Engineering College</span>,
                    graduating in <span className="text-sys-text-primary font-bold">2027</span>.
                </motion.p>
                ...
                <div className="relative flex justify-center items-end w-full lg:w-auto h-[38vh] md:h-[48vh] lg:h-[58vh]">
                    ...
                    <motion.div style={{ x: imgX, y: imgY }} className="relative z-10 h-full">
                        <motion.img
                            ...
                            src={heroImg}
                            alt={`${firstName} — Electrical & Electronics Engineering Student`}
                            draggable={false}
                    ...
                    {[
                        { label: 'React', Icon: SiReact, iconColor: '#61DAFB', pos: 'absolute -left-6 top-[18%]', delay: 0.9 },
                        { label: 'Firebase', Icon: SiFirebase, iconColor: '#FFCA28', pos: 'absolute -right-4 top-[28%]', delay: 1.0 },
                        { label: 'ESP32', Icon: SiEspressif, iconColor: '#E53935', pos: 'absolute left-[8%] top-[8%]', delay: 1.1 },
                        { label: 'OpenCV', Icon: SiOpencv, iconColor: '#5C3EE8', pos: 'absolute right-[6%] top-[12%]', delay: 1.2 },
                        { label: 'YOLOv8', Icon: SiYolo, iconColor: '#7C4DFF', pos: 'absolute left-[12%] top-[55%]', delay: 1.3 },
                        { label: 'Flask', Icon: SiFlask, iconColor: '#111827', pos: 'absolute right-[10%] top-[58%]', delay: 1.4 },
                        { label: 'Supabase', Icon: SiSupabase, iconColor: '#3ECF8E', pos: 'absolute left-[20%] top-[72%]', delay: 1.5 },
                        { label: 'MATLAB', Icon: MdFunctions, iconColor: '#0066CC', pos: 'absolute right-[18%] top-[75%]', delay: 1.6 },
                        { label: 'GitHub', Icon: SiGithub, iconColor: '#111827', pos: 'absolute left-[35%] top-[5%]', delay: 1.7 },
                    ].map(({ label, Icon, iconColor, pos, delay }) => (
                        <motion.div key={label}
                            className={`${pos} hidden lg:flex items-center gap-1.5 rounded-full px-3 py-1.5 z-20 shadow-md`}
                            style={{ background: '#ffffff' }}
                            initial={{ opacity: 0, scale: 0.5 }}
                            animate={{ opacity: 1, scale: 1 }}
                            transition={{ delay, type: 'spring', stiffness: 200 }}
                            whileHover={{ scale: 1.1, y: -2 }}>
                            <Icon className="h-[13px] w-[13px]" style={{ color: iconColor }} aria-hidden="true" />
